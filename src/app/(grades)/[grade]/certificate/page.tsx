...
export default function CertificatePage() {
  ...
  const { isLoading: progressLoading, getCertificateEligibility } = useProgress();
  ...
  if (!currentUser) {
     return (
      <div className="text-center py-10 max-w-lg mx-auto px-4">
        ...
        <Button onClick={() => router.push(`/auth/login?redirect=/${gradeId}/certificate`)} size="lg" className="text-sm sm:text-base">
          Iniciar Sesión
        </Button>
      </div>
    );
  ...
  const isEligible = getCertificateEligibility(gradeId);
  ...
  if (!isEligible) {
    return (
      <div className="text-center py-10 max-w-lg mx-auto px-4">
        ...
        <p className="text-muted-foreground mb-8 text-sm sm:text-base">
          No has completado todos los módulos requeridos para {grade.title}. 
          ¡Sigue esforzándote y completa todos los módulos para obtener tu certificado!
        </p>
        <Button onClick={() => router.push(`/${gradeId}`)} size="lg" className="text-sm sm:text-base">
          <ArrowLeft className="mr-2 h-5 w-5" /> Volver a Módulos de {grade.title}
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 no-print">
        <Button variant="outline" onClick={() => router.push(`/${gradeId}`)} className="flex items-center self-start sm:self-center text-xs sm:text-sm">
          <ArrowLeft className="mr-2 h-4 w-4" /> Volver a {grade.title}
        </Button>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold font-headline text-primary text-center sm:text-left">Tu Certificado de Estudios Filosóficos</h1>
